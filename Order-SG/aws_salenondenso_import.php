...
<link rel="stylesheet" href="css/demos.css">   
	...
	$(document).ready(function() {
        ...
        $('#upagain').click(function(){
            $.ajax({
                type: 'POST',
                url: 'db/excel/replace-all-salenondenso.php',
                data: {"action" : "delete"},
...
function hide(){
    window.location.href = "aws_salenondenso_import.php";
...
<div id="isi">
    ...
    <div id="twocolRight">
    <span class="arial12Bold" height="40">2 <sup>nd</sup> Customer Sales Condition MA(NON DENSO) </span><br/><br/>
            <span height="40" id="formatexcel">Download format excel here : <a href="prototype/customersalenondenso.xls" target="_blank" download ><img src="images/csv.jpg" width="16" height="16" border="0"></a></span>
            <form method="POST" enctype="multipart/form-data" name="uploadForm" id="frmimport" action='db/excel/upload-salenondenso.php' >
            <table class='arial11'>
                <tr height="40">
                    <td>Upload</td>
                    <td>:</td>
			...
			<?
              include "../db/conn.inc";
			   ...
			   $query = "DELETE FROM `supawsexc_temp` WHERE `Owner_Comp` = '$comp';";
                ...
                if($flag=='E'){
                    ...
                    $msgtbl.="<table width='100%' border='1' class='tbl1' cellspacing='0' cellpadding='0'>
 						 <tr class='arial11whitebold' bgcolor='#AD1D36'>
                            <th width='10%' scope='col'>Supplier Code</th>
                            <th width='20%' scope='col'>Part Number</th>
                            <th width='10%' scope='col'>2<sup>nd</sup> Customer Group</th>
                            ...
                            <th width='20%' scope='col'>Error Message</th>
                        </tr>";
                    $qse="SELECT * FROM supawsexc_temp WHERE Owner_Comp='$owner_comp' and cusno1='$cusno' ";
					//echo $qse;
                    ...
                    while($arx=mysqli_fetch_array($sqlqse)){
						...
						$sell=$arx['sell'];
						$cusgrp=$arx['cusgrp'];
						$price=number_format($arx['price'],2);
						$curr=$arx['curr'];
						$error=$arx['error'];
                        switch ($sell) {
                            case "1":
                        ...
                        $msgtbl.="<tr class='arial11black'>
                            ...
                            <td style=\"padding-left:5px;\" align=\"left\">$prtno</td>
                            <td align=\"Center\">$cusgrp</td>
                            <td align=\"Center\">$selltext</td>
                            <td align=\"Center\">$curr</td>
                            <td style=\"padding-right:5px;\" align=\"Right\"  style=\"padding-right: 5px;\">$price</td>
                            <td>$error</td>
                        </tr>";
                    }
                    $msgtbl.="</table>";
                ...
                if($msg=='Confirm'){
                    // Check count 
                    $qc1="SELECT COUNT(*) AS fcount FROM supawsexc_temp WHERE Owner_Comp='$comp' and cusno1='$cusno'";
                    ...
                    ?>
                        <form method="POST" enctype="multipart/form-data" action="db/excel/replace-all-salenondenso.php">
                    ...
                    <?
                    $qa2="SELECT * FROM supawsexc_temp WHERE Owner_Comp='$owner_comp' and cusno1='$cusno' LIMIT 10";
                        //echo $qa2;
                        $sqlqa2=mysqli_query($msqlcon,$qa2);
                        while($arrqa2=mysqli_fetch_array($sqlqa2)){
                            $supcode=$arrqa2['supcode'];
