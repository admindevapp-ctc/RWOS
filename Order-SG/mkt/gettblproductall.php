<?php 
	...
	//total Row Count
	$sql = " SELECT  Product, Subprod, orderdate , Corno , sum( qty ) as ttlqty , curcd, sum( bprice * qty ) as ttlamount , sum( bprice * qty * SGPrice ) as ttlamountsg, orderno 
	...
	$sql = $sql. " GROUP BY Product, subprod, orderdate, corno ORDER BY Product, subprod, orderdate, corno";
...
$sQuery=  " SELECT  orderdtl.Owner_Comp, CUST3, Product, Subprod, orderdate , Corno , sum( qty ) as ttlqty , curcd, sum( bprice * qty ) as ttlamount , sum( bprice * qty * SGPrice ) as ttlamountsg, orderno 
FROM orderdtl INNER JOIN bm008pr ON orderdtl.partno = bm008pr.itnbr AND orderdtl.Owner_Comp = bm008pr.Owner_Comp ".
" where orderdtl.Owner_Comp='$comp' and orderdtl.orderdate>='$datefrom' and orderdtl.orderdate<='$dateto'" ;
if($vcussel!=''){
...
$sQuery= $sQuery . " GROUP BY Product, subprod, orderdate, corno ORDER BY CUST3, Product, subprod, orderdate, corno"	;
	...
	while ( $aRow = mysqli_fetch_array( $rResult ) )
	{
		if($i=="0"){
			...
			echo "<td width=\"7%\"  align=\"center\">".$owner_comp. "</td>";
			echo "<td width=\"15%\" align=\"center\">".$product. "</td>";
			echo "<td width=\"15%\" align=\"center\">".$subprod. "</td>";
