                ...
                $("#dialog-form").dialog({
                    ...
                    buttons: {
                        ...
                        '<?php echo get_lng($_SESSION["lng"], "L0075"); /* Save Order Detail */ ?>': function () {
                            ...
                            var shpCd = $('#shpCd').val();
                            edata = "partno=" + epartno + "&orderno=" + orderno + "&corno=" + corno + "&shpno=" + vcusno + "&oecus=" + oecus + "&shipment=" + shipment + "&qty=" + quantity + "&action=" + vaction + "&ordertype=" + ordertype + "&passDueDate=" + passDueDate + "&shpCd=" + shpCd + "&chktf=" + $('#hid_chk_tf').val();
                                        ...
                                        if (data.substr(0, 5) === 'Error' || data.substr(0, 7) === 'ผิดพลาด') {
											...
											if (vaction == 'add') {
												$('#myTable > tbody:last').append("<tr>" +
													...
													"<td class=\"price\" align=\"center\">" + discratio + "</td>" +
													"<td class=\"price\" align=\"center\">" + acsale + "</td>" +
											...
											if (vaction == 'add') {
												$('#myTable > tbody:last').append("<tr>" +
													...
													"<td id=\"duedt\" align=\"center lasttd\">" + duedt + "</td>" +
													"</tr>");
											} else {
                ...
                $("#dialog-progress").dialog({
                    ...
                    buttons: {
                        '<?php echo get_lng($_SESSION["lng"], "L0372"); /* OK */ ?>': function () {
                            var url = "<?php echo "main.php?" . paramEncrypt("ordertype=Request") ?>"; /* Edit by CTC Sippavit 01/10/2020 */
                            $(location).attr('href', url);
                            $(this).dialog("close");
                        }
                    },
...
$query = "select * from `shiptoma` st,`orderhdr` od where  trim(st.Cusno) ='" . $vcusno . "' and st.ship_to_cd = '" . $shpCd . "' and st.Owner_Comp='$comp'";
                ...
                $("#cmdClose").click(function () {
                    ...
                    if (answer) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
//                                $('#result').html(data);
                                window.location.href = 'cata_cataloguemain.php'; /* Edit by CTC Sippavit 09/10/2020 */
                            }
                        });
                    }
                });
    ...
    <body>
        ...
        <div id="isi">
            <div id="twocolRight1">
                        ...
                        <td colspan="5">
                            <table width="97%" border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    $query = "select ship_to_nm,adrs1,adrs2,adrs3,pstl_cd,comp_tel_no from `shiptoma` where trim(ship_to_cd) =trim('" . $shpCd . "') and trim(Cusno) = trim('" . $vcusno . "') and Owner_Comp='$comp'";
                ...
                <table width="97%" border="0" cellspacing="0" cellpadding="0">
                    <tr align="right">
                        <td width="10%" ><input type="button" value="<?php echo get_lng($_SESSION["lng"], "L0413"); /* Save Order Entry */ ?>" id="cmdSave" class="arial11blackbold" /></td>
					...
					<?
						...
						?>
							<tr align="center" valign="middle"  bgcolor="#990033" class="arial11whitebold" >
								...
								<th width="25%" ><?php echo get_lng($_SESSION["lng"], "L0407"); ?><!--Part Number--></th>
								<th width="15%" ><?php echo get_lng($_SESSION["lng"], "L0410"); ?><!--Qty--></th>
								<th width="8%" ><?php echo get_lng($_SESSION["lng"], "L0408"); ?><!--Curr--></th>
						...
						<?php	
						 ...
						 if ($chk_tf) {
							$sql = "select a.*, b.SLS_PRCE, b.SLS_AMNT, b.BS_PRCE, b.DSCNT_RATIO  FROM $table a LEFT JOIN tf_snd_web_item_ma_tk0 b on a.Owner_Comp = b.OWNER_COMP and a.partno = b.CST_ORDR_ITEM_NO_DSP AND a.cusno = b.CST_CD where trim(a.cusno) ='" . $vcusno . "' and trim(a.orderno)='" . $xordno . "' and a.Owner_Comp='$comp' AND b.SHP_TO_CD LIKE '" . $shpCd . "' order by a.partno";
							...
							foreach ($result as $hasil) {
								...
								echo "<td class=\"price\" align=\"center\">" . number_format($ac_slsprice, 2, ".", ",") . "</td>";
							...
							$sql = "select a.* from $table a where trim(a.cusno) ='" . $vcusno . "' and trim(a.orderno)='" . $xordno . "' and Owner_Comp='$comp' order by a.partno";
							$sth = $ctcdb->db->prepare($sql);
							$sth->execute();
							$result = $sth->fetchAll(PDO::FETCH_ASSOC);
							...
							foreach ($result as $hasil) {
								...
								echo "<td class=\"qty\" align=\"center\">" . $qty . "</td>";
								echo "<td class=\"curcd\" align=\"center\">" . $curcd . "</td>";
								echo "<td class=\"price\" align=\"center\">" . $vbprice . "</td>";
								echo "<td class=\"ttl\" align=\"center\">" . $ttl . "</td>";
								echo "<td id=\"duedt\" class=\"lasttd\" align=\"center\">" . $duedt . "</td>";
								echo "</tr>";
							}
