...
<?php } ?>
    modal: true,
            ...
            buttons: {//Save Order Header
            "<?php echo get_lng($_SESSION["lng"], "L0044"); ?>": 
            function() {
                var bValid = true;
				...
				if (str2.length == 0){
                    $('#requestDate').addClass("ui-state-error");
                    updateTips("1st Customer's leadtime setting not found.");
                    return false;
                }
            ...
            "<?php echo get_lng($_SESSION["lng"], "L0045"); ?>": 
                function() {
                    $(this).dialog("close");
                }
            },
...
<?php } ?>
        ...
        success: function(res) {
            ...
            if(res.length>0){
                for(var i=0;i<res.length;i++){
                    $("#txtShpNo").append("<option value='"+res[i].shipCd+"'>"+res[i].shipCd+" : "+res[i].shipNm+"</option>");
                }
            }
            if(res.length==1){
    ...
    type: 'GET',
			...
			if(rcv[3] == '1'){
				if(rcv[4] == '') {
					rcv[4] = '<? echo get_lng($_SESSION["lng"], "E0027") ;?>';
				}
				$(".validateTips2").html(rcv[4]).addClass("ui-state-highlight");
        ...
        <div id="twocolRight">
            ...
            <table width="100%" class="tbl1" cellspacing="0" cellpadding="0">
                <tr class="arial11white" bgcolor="#AD1D36" >
                    ...
                    $query = "select * from awsorderhdr  where trim(CUST3) ='" . $cusno . "'
                        and Owner_Comp='$comp' and  ordflg = ''";
...
/* zia Added */							
                        ...
                        if ($custype == "A") {
                            switch (trim($ordflg)) {
                                case "":
                                    $sts = get_lng($_SESSION["lng"], "L0566");
                                    break;
                                case "R":
                                    $sts = "Rejected";
                                    break;
                    ...
                    if ($mcount > $mper_page) {
                        echo "<tr height=\"30\">";
                        echo "<td colspan=\"" . $colnum . "\" align=\"right\" class=\"lasttd\"><div id=\"pagination\" >";
                        $fld = "mpage";
                        //pagingfld($query, $mper_page, $num, $mpage, $fld);
                        paging($query, $mper_page, $num, $mpage, $fld);
                        echo "</div></td></tr>";
                    }
                ...
                <form>
                    <table width="97%" border="0"   cellspacing="0" cellpadding="0">
                        ...
                        <?php if ($ordertype == 'Request') { ?>
                            ...
                            <tr class="arial11blackbold">
                                <td>&nbsp;</td>
                                <td><span class="arial11redbold"><?php echo get_lng($_SESSION["lng"], "L0065"); ?><!--Due Date--></span></td>
                                <td>:</td>
                                <td  class="arial11blackbold"><input name="requestDate" id="requestDate" type="text" size="12" maxlength="12" ></td>
                        ...
                        <tr class="arial11blackbold">
                            ...
                            <?
                            $qryshp="SELECT cusmas.Cusno, cusmas.ESCA1, cusmas.ESCA2, cusmas.ESCA3, cusmas.OECus, cusrem.curcd, cusrem.remark 
                                FROM `cusmas` LEFT JOIN cusrem ON cusmas.cusno = cusrem.cusno AND cusmas.Owner_Comp = cusrem.Owner_Comp  where  trim(cusmas.cust3) ='$cusno' and cusmas.Owner_Comp='$comp' order by cusmas.Cusno" ;
                            //echo $qryshp;
                            ...
                            ?>
                                <select name="txtShpNo" id="txtShpNo" style="width: 300px" onchange="getShipToAddressAjax(this.value)">
                                </select>
                            </td>
                        </tr>
                        <tr class="arial11blackbold">
