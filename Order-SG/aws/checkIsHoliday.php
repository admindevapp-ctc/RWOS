<?php
session_start();
require_once('../../language/Lang_Lib.php');
require('../db/conn.inc');
require_once('../../core/ctc_init.php'); // add by CTC

$selected = trim($_GET['selected']);
$dealer = $_SESSION['dealer'];
$cusno =	$_SESSION['cusno'];

$parts = explode('-', $selected);
$desc = '';
$comp = ctc_get_session_comp(); // add by CTC
$setRdayQry = "select * from awsduedate where ordtype='R' and Owner_Comp='$comp'  AND awsduedate.cusno = '" . $dealer . "'";
	$setRdaysql = mysqli_query($msqlcon, $setRdayQry);
	$setRday = "";
	if ($resultR = mysqli_fetch_array($setRdaysql)) {
		$setRday = $resultR['setday'];
		$holiday_chk = $resultR['holiday_st'];
	}

	if($holiday_chk == 1){
		$qryIsHoliday = "select * from supsc003pr where yrmon='" . $parts[2] . $parts[1] . "' and Owner_Comp='$comp' ";
		$sqlResult = mysqli_query($msqlcon,$qryIsHoliday);
		if ($ArrayDue = mysqli_fetch_array($sqlResult)) {
			$calcd = $ArrayDue['calcd'];
			$todayIsWrkDay = substr($calcd, ($parts[0] - 1), 1); // check today is holiday
			if ($todayIsWrkDay == 1) {
				
			} else {
				$desc = get_lng($_SESSION["lng"], "E0035")/*'Error: Selected Due Date on DENSO Holiday'*/;
			}
		} else {
			$desc = get_lng($_SESSION["lng"], "E0006")/*'Error: Calender was not found, Please contact DSTH'*/;
		}
	}else{
		
	}

echo $desc;
?>