	...
	<tbody>
	  ...
	  if(isset($_POST["cust3"]) && $_POST["cust3"]!="" && $_POST['prdfrom']!=""){
		if($_POST['prdto']!="" && $_POST['prdto']<$_POST['prdfrom']){
			echo "<tr><td colspan='8' align='center' class='arial21redbold'>Please choose Customer 3 </td></tr>";					  
		 }else{
			if($_POST['prdto']==""){
			...
			$find=$_POST['Optby'];
			echo  "<span class='arial12Bold '> Seach Result   : </span><span class='arial11white'> CUST3 = ". $cust3 . " and periode from =". $prdfrom . " and periode to=".$prdto ."</span>";
	  	if($cust3=='All'){
		...
		while($hasil = mysqli_fetch_array ($sql)){
			...
			if($cust3=='All'){
					$query2="SELECT PROD, SUM(QTY) as planqty, SUM(AMOUNT)as planamt from slsplan where YYYYMM >= $prdfromac and  YYYYMM <= $prdtoac   and PROD='$product' Owner_Comp='$comp' group by PROD order by PROD";
			}else{
					$query2="SELECT PROD, SUM(QTY) as planqty, SUM(AMOUNT)as planamt from slsplan where YYYYMM >= $prdfromac and  YYYYMM <= $prdtoac  and CUST3 ='$cust3' and PROD='$product' Owner_Comp='$comp' group by PROD order by PROD";		
			}
