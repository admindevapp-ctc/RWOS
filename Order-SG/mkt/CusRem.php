...
<script type="text/javascript">
	...
	<body >
		...
		if(trim($_GET['action'])=='add'){
			...
			$query="select Cusno, Cusnm from cusmas where Cusno not in (select cusno from cusrem) ";
			...
			$inpremark="<input type=\"text\"  name=\"vremark\" class=\"arial11black\" maxlength=\"45\" size=\"45\"></input>";
			$inpcurcd="<input type=\"text\"  name=\"vcurcd\" class=\"arial11black\" maxlength=\"10\" size=\"10\"></input>";
	    	$inpaction="<input type=\"hidden\" name=\"vaction\" value=".$action."></input>";			
	
		}else{
			$vcusno=trim($_GET['cusno']);
			$query="select * from cusrem inner join cusmas on cusrem.cusno=cusmas.cusno where cusrem.Cusno='". $vcusno."'";
		//echo $query;
			$sql=mysqli_query($msqlcon,$query);	
