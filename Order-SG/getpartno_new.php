				...
				if($sisa==0){
					...
					if($err!='1'){
						...
						if($chk_tf != '1'){
							...
							$desc=$partdes."||".$bprice."||".number_format($disc_ratio). " % "."||".number_format($slsamnt,2,".",",")."||".$qty."||".$curcd."||".$vttlprice."||".$duedate_format2."||".$vttlex;		
					}
					...
					if($hasil2 = mysqli_fetch_array ($sql2)){
						if($action=='add'){
							...
							$query3="insert into ".$table. " (CUST3, orderno,orderdate,cusno,partno,partdes,ordstatus,qty,CurCD, bprice, SGCurCD, SGPrice,disc,dlrdisc,slsprice,Corno,DueDate, DlrCurCD, DlrPrice, OECus, Shipment,Owner_Comp) values('$cusno','$orderno','$YMD','$shpno','$partno','$partdes','$ordertype',$qty, '$curcd', $bprice,'SD', $exrate, $disc, $dlrdisc,$slsprice, '$corno', '$duedate_format1', '$curcddlr', $bpricedlr, '$oecus', '$shipment','$comp')";
							//echo $query3;
