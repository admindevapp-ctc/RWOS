...
header("Location:../login.php");
	...
	*/
	...
	$sql = "SELECT * from suporderhdr 
		inner join suporderdtl on suporderhdr.orderno=suporderdtl.orderno and suporderhdr.Owner_Comp=suporderdtl.Owner_Comp 
		where suporderhdr.orderdate>='$datefrom' 
        and suporderhdr.orderdate<='$dateto' and suporderhdr.Owner_Comp='$owner_comp' and suporderhdr.supno = '".$supno."' ";
	//echo $sql;
...
//			  " where orderhdr.cust3='$cusno' and orderhdr.orderdate>='$datefrom' and orderhdr.orderdate<='$dateto'" ;
	...
	" where suporderhdr.orderdate>='$datefrom' and suporderhdr.orderdate<='$dateto' and suporderhdr.Owner_Comp='$owner_comp' and suporderhdr.supno = '".$supno."'" ;
...
//Zia Added for Order Progress	> End
		...
		$odrsts=$aRow['ordtype'];
			...
			echo "<td width=\"10%\" align=\"center\">".$corno."</td>";
				...
				echo "<td width=\"10%\" align=\"right\">".number_format($slsprice,2)."</td>";
				echo "<td width=\"10%\" align=\"right\">".number_format($qty * $slsprice,2)."</td>";  
				echo "<td width=\"10%\" align=\"center\">".$supno."</td>";  
			echo "<td width=\"15%\" class=\"lasttd\"><center>";
			echo " <input type=\"text\" class=\"form-control\" maxlength=\"50\" id='txt_remark".$orderno."".$supno."".$owner_comp."".$corno."".$vcust3."' value='".$remark."' style=\"display:inline-block;width:120px;\"/>";
			echo " <img src=\"../../images/save.png\" class='saveremark' id='".$orderno."|".$supno."|".$owner_comp."|".$corno."|".$vcust3."|' style=\"vertical-align: middle;\">";
			echo "</center></td>"; 
			echo "</tr>";
				}
	if($i=="1") echo "</table>";
