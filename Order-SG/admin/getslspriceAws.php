...
{       
	...
	if (! $aRow = mysqli_fetch_array( $result ) ){
		...
		if (! $aRow = mysqli_fetch_array( $result ) ){
			...
			$sql = "SELECT * from sellpriceaws where itnbr='$itnbr'and cusno='$shpno' and Owner_Comp='$comp'"  ;
			$result = mysqli_query($msqlcon,$sql);
			if (! $aRow = mysqli_fetch_array( $result ) ){
				if($action=='add'){
				   $sql = "insert into sellpriceaws(Cusno, Itnbr, PriceAWS, CurCDAWS, Price, CurCD, DlrCD, Owner_Comp) values('$shpno', '$itnbr', $price, '$curcd', $dprice, '$dcurcd', '$dlrcd','$comp')" ;
				   $result = mysqli_query($msqlcon,$sql);	
				}else{
				...
				if($action=='add'){
					...
					if($action=='edit'){
						$sql = "update sellpriceaws set PriceAWS= $price,  CurCDAWS='$curcd', price=$dprice, CurCD='$dcurcd', DlrCD='$dlrcd'  where cusno='$shpno'  and Itnbr='$itnbr' and Owner_Comp='$comp'" ;
						echo $sql;
						$result = mysqli_query($msqlcon,$sql);
					}else{
						if($action=='delete'){
							$sql="delete from  sellpriceaws  where cusno='$shpno'  and Itnbr='$itnbr' and Owner_Comp='$comp'" ;
							$result = mysqli_query($msqlcon,$sql);
