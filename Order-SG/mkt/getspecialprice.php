...
header("Location:../../login.php");
	...
	if (! $aRow = mysqli_fetch_array( $result ) ){
		...
		if (! $aRow = mysqli_fetch_array( $result ) ){
			...
			$sql = "SELECT * from specialprice where itnbr='$itnbr'and cusno='$shpno'"  ;
			$result = mysqli_query($msqlcon,$sql);
			if (! $aRow = mysqli_fetch_array( $result ) ){
				if($action=='add'){
				   $sql = "insert into specialprice values('$shpno', '$itnbr', $price, '$curcd', '$cust2', '$cust3')" ;
				...
				if($action=='add'){
					...
					if($action=='edit'){
						$sql = "update specialprice set Price= $price,  curcd='$curcd'  where cusno='$shpno'  and Itnbr='$itnbr'" ;
						echo $sql;
						$result = mysqli_query($msqlcon,$sql);
					}else{
						if($action=='delete'){
							$sql="delete from  specialprice  where cusno='$shpno'  and Itnbr='$itnbr'" ;
							$result = mysqli_query($msqlcon,$sql);
