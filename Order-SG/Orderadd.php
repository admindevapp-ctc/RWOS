...
function checknew($vcusno, $vcusnm, $vcorno, $vordno, $vshpno, $vorddate, $action, $oecus, $vshipment, $vordtype, $requestDate, $shpCd , $txtnote) {
    ...
    $comp = ctc_get_session_comp(); // add by CTC
    // Check PO Number
    $query = "select * from orderhdr where Cusno = '" . $vshpno . "' and Corno='" . $vcorno . "' and Owner_Comp='$comp' ";  // edit by CTC
    $sql = mysqli_query($msqlcon,$query);
    $hasil = mysqli_fetch_array($sql);
    if (!$hasil) {
        $query = "select * from orderhdr where orderno='" . $vordno . "' and Owner_Comp='$comp'";  // edit by CTC
        $sql = mysqli_query($msqlcon,$query);
        $hasil = mysqli_fetch_array($sql);
        if (!$hasil) {
            ...
            } else if ($vordtype == "Advance") {
                echo "Order2.php?" . paramEncrypt("action=$action&ordno=$vordno&cusno=$vcusno&orddate=$vorddate&corno=$vcorno&shpno=$vshpno&oecus=$oecus&shipment=$vshipment&ordertype=$vordtype");
            }
        } else {
