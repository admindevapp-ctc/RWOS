...
function supsendmailAttachFile($to ,  $orderno , $corno , $cusno , $cusnm , $bcc, $supno, $supnm, $emailcus){//CTC P.Pawan 04/03/19 email add attach file
	$mail = new PHPMailer();
	$mail->IsHTML(true);
	$mail->IsSMTP(true);
	$mail->CharSet = "utf-8";
	...
	if(isset($to) && !empty($to) && $to != ""){
		$file = generatesupPDF($orderno,$cusno,$path, $supno);
		// $file = "";
		if(empty($file) || $file===""){
			$subject = "**DENSO Order Mail Sent Error ".$corno;
			$content ="Dear Administrator<br>";
	...
	if (!$mail->send()) {
		if(isset($bcc)){
			supsendmailAttachFile($bcc ,  $orderno , $corno , $cusno , $cusnm,null, $supno,$supnm);
		}
    	echo '{"result":"fail"}';
	} else {
		if(isset($bcc)){
...
if(isset($_POST['axEmail']) && isset($_POST['orderNo']) && isset($_POST['corno']) && isset($_POST['cusno']) && isset($_POST['cusnm']) && isset($_POST['cusno']) && isset($_POST['supno']) && isset($_POST['supnm']) ){//CTC P.Pawan 04/03/19 ajax email add attach file
	// require('generatePDF.php');  // CTC move to function supsendmailAttachFile 
	$to = $_POST['axEmail'];
	...
	$supnm =  $_POST['supnm'];
	$cc =  $_POST['cc'];
	supsendmailAttachFile($to ,  $orderno , $corno , $cusno , $cusnm ,$bcc, $supno, $supnm, null);
