...
function generateAWS_DensoPDF($ordno,$corno,$approvetype,$cusno,$path){
    ...
    if($approvetype == "1"){
		$sql =  "SELECT *, orderdtl.ordflg as flgorder , awscusmas.ship_to_cd2 as 'shipto'".
		" from orderhdr ". 
		" LEFT join orderdtl on orderhdr.orderno=orderdtl.orderno and orderhdr.corno=orderdtl.corno and  orderhdr.Owner_Comp=orderdtl.Owner_Comp ".
		" LEFT join cusmas on orderhdr.cusno=cusmas.cusno and orderhdr.Owner_Comp=cusmas.Owner_Comp ".
		" LEFT JOIN awscusmas ON orderhdr.Owner_Comp = awscusmas.Owner_Comp AND orderhdr.cusno = awscusmas.cusno2 AND orderhdr.Dealer = awscusmas.cusno1 AND orderhdr.shipto = awscusmas.ship_to_cd1 ".
		" LEFT join shiptoma on orderhdr.shipto=shiptoma.ship_to_cd and orderhdr.Dealer =shiptoma.cusno and orderhdr.Owner_Comp=shiptoma.Owner_Comp".	
		...
		$sql = "SELECT *, awsorderdtl.ordflg as flgorder 
			from awsorderhdr 
			LEFT join awsorderdtl on awsorderhdr.orderno=awsorderdtl.orderno and awsorderhdr.corno=awsorderdtl.corno and  awsorderhdr.Owner_Comp=awsorderdtl.Owner_Comp 
			LEFT join cusmas on awsorderhdr.cusno=cusmas.cusno and awsorderhdr.Owner_Comp=cusmas.Owner_Comp 
			LEFT JOIN awscusmas ON awscusmas.cusno2 = awsorderhdr.cusno AND awscusmas.Owner_Comp = awsorderhdr.Owner_Comp AND awscusmas.ship_to_cd2 = awsorderhdr.shipto 
			LEFT JOIN shiptoma ON awscusmas.ship_to_cd1 = shiptoma.ship_to_cd AND awscusmas.cusno1 = shiptoma.cusno AND awsorderhdr.Owner_Comp = shiptoma.Owner_Comp
