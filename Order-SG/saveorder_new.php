...
if ($error) {
	...
	if($action=='edit'){
		...
		$query8="select orderdtl.*, availablestock.prtno  from orderdtl inner join availablestock on orderdtl.partno=availablestock.prtno and orderdtl.Owner_Comp=availablestock.Owner_Comp where trim(orderno)='".$orderno."' and trim(cusno)='".$shpno."' and corno='".$corno."' and orderdtl.Owner_Comp='".$comp."' ";
		...
		mysqli_query($msqlcon,$query5);
		$query6="delete from ordernts where trim(orderno)='".$orderno."' and trim(cusno)='".$shpno."' and corno='".$corno."' and Owner_Comp='".$comp."' ";
	...
	// test section
			...
			// if($chk_tf){
			// echo $emp1_query = "
					// SELECT a.*, b.SLS_PRCE, b.SLS_AMNT, b.BS_PRCE, b.DSCNT_RATIO  FROM $table a LEFT JOIN tf_snd_web_item_ma_tk0 b on a.Owner_Comp = b.OWNER_COMP and a.partno = b.CST_ORDR_ITEM_NO_DSP AND a.cusno = b.CST_CD WHERE TRIM(a.cusno) = '$shpno' AND TRIM(a.orderno) = '$orderno' AND a.Owner_Comp = '$comp' AND b.SHP_TO_CD LIKE '$shpCd' ORDER BY a.partno 
			// ";
			// }else{
	...
	if($action!='close'){
		...
		if($chk_tf){
			$query = "
			SELECT a.*, b.SLS_PRCE, b.SLS_AMNT, b.BS_PRCE, b.DSCNT_RATIO  FROM $table a LEFT JOIN tf_snd_web_item_ma_$comp b on a.Owner_Comp = b.OWNER_COMP and a.partno = b.CST_ORDR_ITEM_NO_DSP AND a.cusno = b.CST_CD WHERE TRIM(a.cusno) = '$shpno' AND TRIM(a.orderno) = '$orderno' AND a.Owner_Comp = '$comp' AND b.SHP_TO_CD LIKE '$shpCd' ORDER BY a.partno";
		}else{
			$query="select * from ".$table. " where trim(orderno)='".$orderno."' and trim(cusno)='".$shpno."' and Owner_Comp='".$comp."' ";
		}
		...
		while($tmphsl=mysqli_fetch_array($sql)){
			...
			if($x=='1'){
				...
				$query2="insert into orderhdr (CUST3, orderno, cusno, ordtype,orderdate,orderprd, corno, ordflg, Dealer, ordflagdate, ordflagtime, OECus, Shipment, shipto,Owner_Comp) values('$cusno', '$orderno','$cusno', '$orderstatus','$orderdate','$periode','$corno','$ordflg','$dealer', '$ordflagdate', '$ordflagtime','$oecus', '$shipment', '$shpCd','$comp')";//12/20/2018 P.Pawan
				mysqli_query($msqlcon,$query2);

				mysqli_query($msqlcon,"SET NAMES UTF8");

				$query7="insert into ordernts (CUST3, orderno, cusno, ordtype,orderdate,orderprd, corno, ordflg, Dealer, ordflagdate, ordflagtime, OECus, Shipment, shipto, notes,Owner_Comp) values('$cusno', '$orderno','$cusno', '$orderstatus','$orderdate','$periode','$corno','$ordflg','$dealer', '$ordflagdate', '$ordflagtime','$oecus', '$shipment', '$shpCd', '$txtnote','$comp')";//12/20/2018 P.Pawan
			...
			$query3="insert into orderdtl (CUST3, orderno, cusno, partno,itdsc, orderdate,qty,CurCD, bprice, SGCurCD,SGPrice, disc, dlrdisc, slsprice,Corno, DueDate, ordflg, DlrCurCD, DlrPrice,Owner_Comp) values('$cusno','$orderno','$shpno', '$partno','$partdes','$orderdate', $qty,'$curcd',  $bprice,'SD', $SGPrice,  $disc, $dlrdisc, $slsprice, '$corno', '$duedate', '$ordflg','$dlrcurcd', $dlrprice,'$comp')";
			...
			if ($ordertype=='Urgent'){
				...
				if($newCount<=0){
					$query9="insert into availablestock(prtno,qty,Owner_Comp) values('".$partno."',((select (l1awqy+l2awqy) from hd100pr where prtno='".$partno."' and Owner_Comp='".$comp."')-$qty),'$comp');";
		...
		$emailTo=[];
		$tmpEmail=[];
		$query = "select * from `shiptoma` st,`orderhdr` od where trim(st.Cusno)=trim('".$cusno."') and trim(st.ship_to_cd)=trim('".$shpCd."') and st.Owner_Comp=od.Owner_Comp and st.Owner_Comp='$comp' ";
		$sql=mysqli_query($msqlcon,$query);
		...
		$bcc = get_config("bcc");//CTC P.Pawan 04/03/19 get bcc email from config file.
		$bcc = explode(";",$bcc);
		sendmailAttachFile($emailTo, $orderno, $corno , $cusno , $cusnm , $bcc);
		//sendemailAndPDFFile 02/14/2019 P.Pawan CTC --- End ---
		require('db/conn.inc');
	}
	...
	if($action=="Reject"){
		...
		for($i=0;$i<count($spartno);$i++){
			$vpartno=$spartno[$i];
			$query="update orderdtl set ordflg='$ordertype' where trim(orderno)='".$orderno."' and trim(partno) ='".$vpartno. "' and Owner_Comp='".$comp."' ";
