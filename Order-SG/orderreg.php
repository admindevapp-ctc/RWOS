...
$vprdyear=trim($_GET['prdyear']);
$vprdmonth=trim($_GET['prdmonth']);
...
case 'editAWS':
  $vcusno=$xcusno;
  checkeditAWS($vshpno, $vordno,  $vcorno, $vorddate, $table,$supno,$shipto2);
  break;
...
function checknew($vcusno,$vcusnm,$vcorno,$vordno,$vorddate,$vshpno,$action){
	...
	//	include "crypt.php";
        $query="select * from orderhdr where Corno='".$vcorno."' and cusno='". $vshpno ."' and Owner_Comp='$comp' ";
...
function checkedit($vshpno, $vordno, $vcorno,$action, $table, $cusno, $ordertype, $vshpCd, $vtxtnote){
	...
	$query="select * from orderhdr inner join orderdtl on orderhdr.orderno=orderdtl.orderno and orderhdr.cusno=orderdtl.cusno and orderhdr.Owner_Comp=orderdtl.Owner_Comp inner join bm008pr on orderdtl.partno=bm008pr.ITNBR and orderdtl.Owner_Comp=bm008pr.Owner_Comp where trim(orderhdr.orderno) = '$vordno' and trim(orderhdr.cusno)='".$vshpno."' and Owner_Comp='$comp' ";
    //echo "<br>". $query;
	$sql=mysqli_query($msqlcon,$query);		
	$x=0;
...
function checkdelete($vshpno, $vordno, $vcorno,$ordertype){
	...
	$query="delete  from orderhdr where trim(orderno) = '$vordno' and trim(cusno)='$vshpno' and trim(Corno)='$vcorno' and Owner_Comp='$comp' ";
	mysqli_query($msqlcon,$query);		
	$query1="delete  from orderdtl where trim(orderno) = '$vordno' and trim(cusno)='$vshpno' and trim(Corno)='$vcorno' and Owner_Comp='$comp' ";
	mysqli_query($msqlcon,$query1);
	/*Zia added Note start*/
	$query3="delete  from ordernts where trim(orderno) = '$vordno' and trim(cusno)='$vshpno' and trim(Corno)='$vcorno' and Owner_Comp='$comp' ";   
...
function checkdeleteAdd($vshpno, $vordno,$vcorno){
	...
	mysqli_query($msqlcon,$query1);
	/*Zia added Note start*/
	$query4="delete  from ordernts where trim(orderno) = '$vordno' and trim(cusno)='$vshpno' and trim(Corno)='$vcorno' and Owner_Comp='$comp' ";
...
function checkeditAWS($vshpno, $vordno,  $corno, $cdate, $table, $supno,$shipto2){
	require('db/conn.inc');
	$action="approval";
    $vprdyear=substr($prd, 0,4);
    $vprdmonth=substr($prd,-2);
    //include "crypt.php";
    $url="OrderAwsdtl.php?".paramEncrypt("action=$action&ordno=$vordno&shpno=$vshpno&orddate=$cdate&corno=$corno&table=$table&supno=$supno&shipto=$shipto2");
           echo "<script>document.location.href='".$url. "';</script>";
...
function ApproveAWS($vshpno, $vordno, $vcorno, $vaction,$cusno){
	...
	$query="update orderhdr set ordflg='". $flag."', ordflagdate='".$YMD."', ordflagtime='".$Jam."', ordflaguser='".$cusno."' where trim(orderno) = '$vordno' and trim(cusno)='".$vshpno."' and trim(Owner_Comp)='".$comp."' ";
    mysqli_query($msqlcon,$query);
...
function checkeditAdd($vshpno, $vordno, $table, $vcorno, $vcusno){
	require('db/conn.inc');
	$action="edit";
	$query="select * from orderhdr inner join orderdtl on orderhdr.orderno=orderdtl.orderno and orderhdr.cusno=orderdtl.cusno  inner join bm008pr on orderdtl.partno=bm008pr.ITNBR where trim(orderhdr.orderno) = '$vordno' and trim(orderhdr.cusno)='$vshpno' and  trim(orderhdr.corno)='$vcorno' and trim(orderhdr.Owner_Comp)='$comp'";
  // echo $query;
$sql=mysqli_query($msqlcon,$query);		
	$x=0;
	while($hasil = mysqli_fetch_array ($sql)){
