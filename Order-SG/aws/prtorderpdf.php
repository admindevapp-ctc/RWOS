...
{       
	...
	$sql = "SELECT *, awsorderdtl.ordflg as flgorder ".
		...
		" inner join awscusmas on awsorderhdr.shipto=awscusmas.ship_to_cd2 and awsorderhdr.cusno=awscusmas.cusno2 and awsorderhdr.Owner_Comp=awscusmas.Owner_Comp  and awsorderhdr.Dealer = awscusmas.cusno1 ".	
		" where awsorderhdr.orderno='".$ordno. "'  and cusmas.CUST3='". $cusno. "' ".
		" and awsorderhdr.corno='".$corno. "' and awsorderhdr.Owner_Comp='".$comp. "'";
	if(!isset($shipto)){
		$sql .= 	"  and awsorderhdr.shipto  ='$shipto'" ;  
	}
	$sql .= 	" order by partno " ; 
		...
		// " where 1 limit 100" ; 
	// /echo $sql;	
	$sql_sum = "SELECT SUM(qty) as 'sumqty', SUM(bprice * qty) as 'sumprice',awsordernts.notes, ".
	" 		SUM(
    ...
    " from awsorderhdr ". 
    " inner join awsorderdtl on awsorderhdr.orderno=awsorderdtl.orderno and awsorderhdr.corno=awsorderdtl.corno and  awsorderhdr.Owner_Comp=awsorderdtl.Owner_Comp ".
	" inner join awsordernts on awsorderhdr.orderno=awsordernts.orderno and awsorderhdr.corno=awsordernts.corno and  awsorderhdr.Owner_Comp=awsordernts.Owner_Comp ".
    " inner join cusmas on awsorderhdr.cusno=cusmas.cusno and awsorderhdr.Owner_Comp=cusmas.Owner_Comp ".
    " inner join awscusmas on awsorderhdr.shipto=awscusmas.ship_to_cd2 and awsorderhdr.cusno=awscusmas.cusno2 and awsorderhdr.Owner_Comp=awscusmas.Owner_Comp  and awsorderhdr.Dealer = awscusmas.cusno1 ".	
    " where awsorderhdr.orderno='".$ordno. "'  and cusmas.CUST3='". $cusno. "' ".
    " and awsorderhdr.corno='".$corno. "' and awsorderhdr.Owner_Comp='".$comp. "' order by partno " ;  
   // echo $sql_sum ;
...
if($count  <= 0){
	...
	//	echo "test";
		if($i=="0"){
			...
			if($haveship_to_nm){
				...
				if($haveadrs1){
					...
					if(!$badrs2&&$haveadrs2){
						...
						$pdf->Cell(88,$newY,$adrs21,0,0,'L');
						$pdf->SetFont('Arial','B',9);
						$pdf->Cell(30,10,'Order Date  :',1,0,'L');
						$pdf->Cell(20,10,$orddate,1,0,'L');
						$pdf->SetFont('tahoma','',7);
						$pdf->Cell(32,$newY,'              ',0,0,'L');
						$pdf->Cell(20,$newY,'',0,1,'L');
						$pdf->Cell(32,$newY,'              ',0,0,'L');
			...
			$y = $pdf->GetY();
			$pdf->SetXY(112.5, $y-5);
			$pdf->Cell(20,5, '(Without Tax)',0,0,'C',0);
			// add by Pasakorn N.
			...
			if($grand_app_qty != 0){
				...
				$pdf->Cell(20,8, $grand_app_qty,1,0,C);
			...
			$pdf->Cell(20,8, 'Due Date',1,1,'C',1);
			$pdf->SetFont('Arial','',7);
			$i="1";
		}		
