...
if(isset($_POST['vCusno1']) && isset($_POST['vPartno'])){
  ...
  //  $table  .= '<th></th>';
    ...
    $query1="SELECT awsexc.Owner_Comp, cusno1,  
    case when sell = 1 then 'Sell' else 'Not Sell' end textsell, cusgrp, price, curr,sell
    FROM awsexc left join bm008pr on trim(awsexc.itnbr) = trim(bm008pr.ITNBR) 
...
//echo $Cusgrp;
    ...
    $query="SELECT awsexc.Owner_Comp, cusno1,  
    case when sell = 1 then 'Sell' else 'Not Sell' end textsell, cusgrp, price, curr,sell
    FROM awsexc left join bm008pr on trim(awsexc.itnbr) = trim(bm008pr.ITNBR) 
    WHERE awsexc.Owner_Comp='$comp' and cusno1 = '".$Cusno1."' and trim(awsexc.itnbr) = '".$Partno."'";
    if(strlen($Cusgrp) == 3){
    ...
    while($hasil = mysqli_fetch_array ($sql)){
      ...
      //  $table  .=    '<td><input type="checkbox" id="chk_boxes" class="chk_boxes" name="chk_boxes" value="'.$hasil['cusgrp'].'"/></td>';
        $table  .=    '<td align="center"><input type="text" class="p_cusgroup" id="p_cusgroup'.$i.'" value="'.$hasil['cusgrp'].'" style="text-align:center;"/></td>';
        $table  .=    '<td align="center"> <input type="text" class="p_currency" id="p_currency'.$i.'" value="'.$hasil['curr'].'" maxlength="2" style="text-align:center;"/></td>';
        $table  .=    '<td align="center"><input type="text" class="p_price" id="p_price'.$i.'" value="'.number_format($hasil['price'],2,'.',''). '" style="text-align:right;"/></td>';
        $table  .=    '<td align="center"><input type="checkbox" id="p_condotion'.$i.'"  class="p_condotion" name="p_condotion" value="'.$hasil['sell'].'"';
        if($hasil['sell'] == "1"){  
            $table  .=  ' Checked'; 
        }
        $table  .=    '/></td>';
