...
function ctc_get_catalogue_table($carmaker, $modelname, $modelcode, $subCatMaker, $subModelName, $search, $subGroup, $order, $orderType, $start, $length, $brand) {
	...
	//    Start condition
    $condition = "WHERE cat.Owner_Comp = '" . $comp . "' AND ( sellprice.Shipto IS NULL OR sellprice.Shipto = '001' )";
    if ($isCarMakerValid) {
...
function ctc_aws_get_catalogue_by_id_cus($id) {
    $ctcdb = new ctcdb();
    $comp = ctc_get_session_comp();
    $cusno = ctc_get_session_cusno();
	...
	$sql ="
		...
		Prtnohis,
		Vincode,
		IFNULL(awsexc.price, '-') AS stdprice,
		awscusmas.cusno2,
		'-' AS Stock,
		awsexc.sell
		FROM
		...
		LEFT JOIN awsexc on awsexc.itnbr = cat.ordprtno and awsexc.Owner_Comp = cat.Owner_Comp AND awsexc.cusno1 = awscusmas.cusno1 and awsexc.sell = '1'
		WHERE
...
function ctc_get_catalogue_table_excus($carmaker, $modelname, $modelcode, $subCatMaker, $subModelName, $search, $subGroup, $order, $orderType, $start, $length, $brand) {
    $ctcdb = new ctcdb();
    $comp = ctc_get_session_comp();
    $cusno = ctc_get_session_cusno();
...
//			. " LEFT JOIN hd100pr_count as hd100pr ON hd100pr.prtno = cat.ordprtno AND hd100pr.Owner_Comp = cat.Owner_Comp"
    ...
    $condition = "WHERE cat.Owner_Comp = '" . $comp . "' AND ( sellprice.Shipto IS NULL OR sellprice.Shipto = '001' ) AND sellprice.Cusno = '" . $cusno . "'";
    if ($isCarMakerValid) {
        if (isset($condition) && trim($condition) !== '') {
            $condition = $condition . " && ";
        }
