...
$tableorder = ctc_get_session_tablenamesup();
...
$(function() {
		...
		$( "#dialog-form" ).dialog({
			...
			buttons: {
				...
				/** Zia autopart list description */	
					...
					//alert(oecus);
					//alert(shipment);
					edata="partno="+epartno +"&orderno="+orderno+"&corno="+corno+"&shpno="+shpno+"&oecus="+oecus+"&shipment="+shipment+"&qty="+quantity+"&action="+vaction+"&ordertype="+ordertype+"&passDueDate="+passDueDate + "&shpCd=" + shpCd +"&supno=" +supno;
	...
	$( "#chg" ).click(function() {
		...
		$( "#cmdSave" ).click(function() {
					...
					edata="orderno="+orderno+"&corno="+corno+"&shpno="+shpno+"&action="+action+"&ordertype="+ordertype+"&shpCd="+shpCd+"&txtnote="+txtnote+"&supno="+supno;//12/20/2018 P.Pawan CTC add parameter shpCd
					//alert(edata);
					<?php
					//Email of Supplier
						$notiTxt = '';
						...
						//set supno
						$query = "select supmas.* from ". $table . " a ".
							" join supmas on a.supno  = supmas.supno " .
							" where cust3 = '" . $vcusno . "' and Corno = '" . $vcorno . "'";
						//echo $query;
						$sql = mysqli_query($msqlcon, $query);

						while ($axData = mysqli_fetch_array($sql)) {
							$prsnMail1 = $axData['email1'];
							$prsnMail2 = $axData['email2'];
						}
						array_push($tmpEmail, $prsnMail1, $prsnMail2);
							...
							success: function(data) {
								...
								//CTC P.Pawan 04/03/19 show message after email sent -- end --
									...
									if(data.indexOf("failPDF")!==-1){
										...
										$("span#ui-id-3.ui-dialog-title").text('<?php echo get_lng($_SESSION["lng"], "L0321"); ?>');//Send email
										$("p[id=notification]").html('');
										$("p[id=notification]").append('<b><?php echo get_lng($_SESSION["lng"], "L0322");/*Email has been sent successfully*/ ?></b><br>');
									}
									$("#dialog-progress").dialog("open");
									*/
		...
		$( "#cmdClose" ).click(function() {
				...
				$.ajax({
						...
						success: function(data) {
							//alert(data);
								//$('#result').html(data);
                                window.location.href = 'suphistory.php'; /* Edit by CTC Sippavit 09/10/2020 */
								}
						});
...
<?php
	...
	require_once('../core/ctc_cookie.php');
	   ...
	   echo "<input type=\"hidden\" name=\"supno\" id=\"supno\" value=".$supno.">";
	   if($cusno==$vshpno){
	   }
        ...
        if($hasil){
			...
			$xordno=$today.$vordno.substr($ordertype,0,1);//YYMMDDXXXXXXXR
 ...
 <table width="97%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr class="arial11blackbold">
    <tr align="center">
		...
		<?php if($ordertype=='Urgent'){require('countdown.php');}?>
		</td>
	</tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  ...
  <tr class="arial11blackbold">
	...
	<table width="97%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		<td width="5%" class="arial11blackbold"><? echo  $shpCd ?><input type="hidden" id="shpCd" value="<? echo $shpCd ?>"/><!--//12/20/2018 P.Pawan CTC--></td>
  ...
  <tr class="arial11blackbold">
    <td style="vertical-align:top;"><?php echo get_lng($_SESSION["lng"], "L0311"); ?><!--Ship To Address //12/20/2018 P.Pawan CTC--></td>
  ...
  <!--Note //09/11/2019 Zia Added Start-->
    ...
    <tr class="arial11blackbold">
    <td style="vertical-align:top;"><?php echo get_lng($_SESSION["lng"], "L0334"); ?><!--Note --></td>
    <td style="vertical-align:top;">:</td>
   ...
   ?>
        ...
        <table width="97%" border="0" cellspacing="0" cellpadding="0">
          <tr align="right">
            ...
            <td width="8%"><input type="button" class="arial11blackbold" id="cmdClose" value="<?php echo get_lng($_SESSION["lng"], "L0058");/*Close order Entry*/ ?>"></input></td>
            <td width="10%">&nbsp;</td>
            <td width="4%">&nbsp;</td>
            <td width="29%"></td>
        ...
        <table class="tblorder"  width="97%" border="0" cellpadding="0" cellspacing="0"  id="myTable">
         <tbody>
          <tr align="center" valign="middle"  bgcolor="#990033" class="arial11whitebold" >
            ...
            <th width="20%" ><?php echo get_lng($_SESSION["lng"], "L0060"); ?><!--Part Number--></th>
            <th width="15%" ><?php echo get_lng($_SESSION["lng"], "L0061"); ?><!--Qty--></th>
            <th width="8%" ><?php echo get_lng($_SESSION["lng"], "L0062"); ?><!--Curr--></th>
            <th width="13%" ><?php echo get_lng($_SESSION["lng"], "L0063"); ?><!--Price--></th>
             <th width="15%" ><?php echo get_lng($_SESSION["lng"], "L0064"); ?><!--Amount--></th>
            <th width="15%" class="lastth"><?php echo get_lng($_SESSION["lng"], "L0065"); ?><!--Due Date<br>(DD-MM-YYY)--></th>

          </tr>
        <?
		require('db/conn.inc');
		//$query="select a.* from  $table a where trim(a.cusno) ='".$vshpno. "' and trim(a.orderno)='".$xordno."' and Owner_Comp='$comp' and supno='$supno' order by a.partno";
		$query = "select distinct a.*,b.Supcd as supno from $tableorder a join supcatalogue b on a.partno = b.Prtno and a.supno = b.Supcd and a.Owner_Comp = b.Owner_Comp where trim(a.cusno) ='" . $vshpno . "' and trim(a.orderno)='" . $xordno . "' and a.Owner_Comp='$comp' order by a.partno";
		//echo $query;
		$sql=mysqli_query($msqlcon,$query);
			...
			echo "<tr>";
			echo "<td><input name=\"chkaction[]\" type=\"checkbox\" class=\"chkaction\" value='" . $partno . "'></td>" ;
			echo "<td class=\"supno\" align=\"center\">".$bsupno."</td>";
			echo "<td>".$partno. "</td>";
			echo "<td class=\"qty\" align=\"right\">".$qty."</td>";
			echo "<td class=\"curcd\" align=\"right\">" . $curcd . "</td>";
	...
	<div class="demo">
   	   ...
   	   <div id="dialog-form" title="" style="display: none;">
				<p class="validateTips"><?php echo get_lng($_SESSION["lng"], "L0036"); ?><!--All form fields are required.--></p>
					...
					<form>
						<fieldset>
							<label for="partno"><?php echo get_lng($_SESSION["lng"], "L0069"); ?><!--Part Number--></label><img id="buffer" src="images/ui-anim_basic_16x16.gif" style="display:none"></img>
							<div class="ui-widget">
									<select id="partno">
									<?php
