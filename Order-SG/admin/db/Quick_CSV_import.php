...
class Quick_CSV_import
{
  ...
  function import()
  {
    ...
    if("" != $this->encoding && "default" != $this->encoding)
      $this->set_encoding();
    /* change end */
    //echo 'file name ='. $this->file_name;
    if($this->table_exists)
    {
      $sql = "LOAD DATA INFILE '".@mysqli_escape_string($this->file_name).
             "' INTO TABLE `".$this->table_name.
             "` FIELDS TERMINATED BY '".@mysqli_escape_string($this->field_separate_char).
             "' OPTIONALLY ENCLOSED BY '".@mysqli_escape_string($this->field_enclose_char).
             "' ESCAPED BY '".@mysqli_escape_string($this->field_escape_char).
             "' ".
             ($this->use_csv_header ? " IGNORE 1 LINES " : "");
             //."(`".implode("`,`", $this->arr_csv_columns)."`)";
      $res = @mysqli_query($msqlcon,$sql);
      //echo $sql;
  ...
  function create_import_table()
  {
    ...
    if(!empty($this->arr_csv_columns))
    {
      ...
      $res = @mysqli_query($msqlcon,$sql);
      $this->error = mysqli_error();
      $this->table_exists = ""==mysqli_error();
    }
  }
  ...
  function create_importtable()
  {
    ...
    if(!empty($this->arr_csv_columns))
    {
      $arr = array();
      for($i=0; $i<sizeof($this->arr_csv_columns); $i++)
          $arr[] = "`".trim($this->arr_csv_columns[$i])."` TEXT";
      $sql .= implode(",", $arr);
      $sql .= ")";
      //echo "sql".$sql;
      $res = mysqli_query($msqlcon,$sql);
