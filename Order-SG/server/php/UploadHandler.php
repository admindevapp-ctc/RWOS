...
// end		
  ...
  // added by emanuel
    protected function handle_form_corno($file, $index) {
        $corno = $_REQUEST['corno'];
        return $corno;
    }
     
    protected function handle_form_shpno($file, $index) {
        $shpno = $_REQUEST['shpno'];
        return $shpno;
    }	
 //-end addition
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            // form data
            $shpno = $this->handle_form_shpno($file, $index);
            $corno = $this->handle_form_corno($file, $index);
          //  $cusno = $this->handle_form_cusno($file, $index);
            ...
            $file->size = $file_size;
           $file->upload_to_db = $this->add_img($shpno,$corno, $file->name);
            $this->set_file_delete_properties($file);
        }
        return $file;
    }
    
    	function add_img($shpno,$corno, $whichimg)  
	{  
		$add_to_db = $this->query("INSERT INTO attachment(cusno, corno, namefile) VALUES
	     ('$shpno','$corno', '$whichimg')") or die(mysqli_error());  
		return $add_to_db;  
	}
	function query($query) {  
		...
		$db_selected = mysqli_select_db($database);  
		if (!$db_selected) {  
			die(mysqli_error());  
		}  
