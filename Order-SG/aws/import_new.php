  ...
  <tr height="5"><td colspan="5"></td><tr>
	...
	// echo "session imptable =" . $_SESSION['imptable'];
		...
		if(basename($_FILES['file']['name'])==""){
			echo "<script>document.location.href='importregorder.php';</script>";
		}
	...
	if (($ext == "xls") && ($_FILES["file"]["size"] < 2000000)) {
		...
		$data = new Spreadsheet_Excel_Reader($_FILES['file']['tmp_name'], true);
			...
			$time="";
			$setdayQry="select * from duedate where ordtype='R' and Owner_Comp='$comp' ";
			...
			{	
							...
							if($mod!=0){
								...
								}else
								{
									...
									} else
									{
										...
										$qry="select * from awsorderhdr where cust3='".$cusno."' and Corno='".trim($PO)."' and Owner_Comp='$comp'";
									 ...
									 //echo $partdes;
										...
										$qry="select * from orderhdr where cust3='".$dealer."' and Corno='".trim($PO)."' and Owner_Comp='$comp'";
										$sqle=mysqli_query($msqlcon,$qry);
										$hsle = mysqli_fetch_array ($sqle);
										if($hsle){
												$errorMsg.=get_lng($_SESSION["lng"], "E0004")/*"Error : PO Number has been found in order history!"*/;
											...
											if(strtoupper($route)=='N'){
												...
												where trim(awsexc.itnbr) = '".trim($PartNo)."'   and awsexc.sell = '1' 
												...
												$query="select distinct awsexc.*   
												from awsexc 
												left join awscusmas on awscusmas.cusgrp = awsexc.cusgrp  and awscusmas.ship_to_cd2='$shpCd'
													and awscusmas.Owner_comp = awsexc.Owner_comp and awscusmas.cusno1 = awsexc.cusno1 
												where trim(awsexc.itnbr) = '".trim($PartNo)."'  and awsexc.sell = '1' 
													and awsexc.Owner_comp = '$comp'
													and awscusmas.cusno2 =  '$shpno'  ";
				...
				$query = "INSERT INTO $imptable(Cust3, Corno, orderdate,duedate, ordprd, cusno,partno,partdes, ordsts, qty
				, CurCD, bprice,SGCurCD, SGPrice, impgrp, DlrCurCD, DlrPrice, OECus, Shipment,Owner_Comp) 
				VALUES ('$cusno', '$PO', '$cymd','$duedate_format1', '$YYYYmm', '$shpno','$PartNo','$partdes','$ordsts',$qty
				,'$curcd',  $price,'SD', $exrate, '$igroup', '$dlrcurcd', $dlrprice, '$oecus', '$shipment','$comp')";
//echo $query . "<br>";
		...
		if($error>0 || $failed>0 || $warning>0){
			...
			echo get_lng($_SESSION["lng"], "E0017")/*Error (cannot be uploaded) */.": ".$error."<br>";
		...
		// get order No
		   ...
		   $query="select * from tc000pr where trim(cusno) = '$cusno' and Owner_Comp='$comp' ";
			...
			while($hasil = mysqli_fetch_array ($sql)){
					...
					$dlrprice = !empty($dlrprice) ? $dlrprice : "NULL";
					$query2="insert into ".$table. " (CUST3, orderno, orderdate,	cusno, partno,	partdes, ordstatus, qty, CurCD, bprice,SGCurCD
					, SGPrice,  disc, dlrdisc, slsprice, Corno, duedate, DlrCurCD, DlrPrice, OECus
					, Shipment,Owner_Comp) values('$cusno', '$xordno','$cdate','$vcusno','$partno'
					,'$partdesc','$ordertype',$qty,'$curcd', $bprice, 'SD',$SGPrice, $disc
					, $dlrdisc,$slsprice,'$corno', '$duedt', '$dlrcurcd', $dlrprice,'$oecus'
					, '$shipment','$comp')";
				//	echo $query2."<br>";
