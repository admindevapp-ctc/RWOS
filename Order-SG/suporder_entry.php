...
<script type="text/javascript">
			...
			function grandtotal() {
				...
				var formattedQty = number_format(sumqty,0);
				$('#myTable').find('td.ttl').each(function() {
				sumttl += parseFloat($(this).text().replace(/,/g, ''));
				});
				$('input.amt-txt').val(formattedQty);
				$('input.ttl-txt').val(number_format(sumttl,2));
			}
			function number_format(nStr,prefix) {
				nStr = parseFloat(nStr).toFixed(prefix);
				nStr += '';
				x = nStr.split('.');
				x1 = x[0];
				x2 = x.length > 1 ? '.' + x[1] : '';
                                        ...
                                        if (data.substr(0, 5) === 'Error' || data.substr(0, 7) === 'ผิดพลาด') {
                                            ...
                                            if (vaction === 'add') {
                                                $('#myTable > tbody:last').append("<tr>" +
                                                        "<td><input name=\"chkaction[]\" type=\"checkbox\" class=\"chkaction\" value=" + partno.val() + "|"+ supno +" /></td>" +
                                                        "<td id=\"supno\">" + supno + "</td>" + 
                                                        "<td id=\"supnm\">" + supnm + "</td>" +
        ...
        //Email of Supplier
            ...
            //set supno
            $query = "select distinct supmas.* from ". $table . " a ".
                " join supmas on a.supno  = supmas.supno and a.Owner_Comp  = supmas.Owner_Comp " .
                ...
                <table class="tblorder"  width="97%" border="0" cellpadding="0" cellspacing="0"  id="myTable">
                    <tbody>
                        ...
                        foreach ($result as $hasil) {
                            ...
                            echo "<tr>";
                            echo "<td><input name=\"chkaction[]\" type=\"checkbox\" class=\"chkaction\" value='" . $partno . "|".$supno."'></td>";
                            echo "<td>" . $supno . "</td>";
                            echo "<td>" . $supname . "</td>";
