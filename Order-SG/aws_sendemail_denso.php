...
require 'aws_config/aws_Web_Lip.php';

function awssendmaildenso($to, $to1,  $title, $content, $from){
...
function awssendmaildensoAttachFile($to ,  $orderno , $corno , $cusno , $cusnm , $bcc, $supno, $supnm, $approvetype){
	$mail = new PHPMailer();
	$mail->IsHTML(true);
	$mail->IsSMTP(true);
	$mail->CharSet = "utf-8";
	...
	$path = get_aws_config("path_AWS_PDF");
	$email_template = get_aws_config("emailTemplate");
	$po_tempalte = get_aws_config("POTemplate");
	...
	if(isset($to) && !empty($to) && $to != ""){
		$file = generateAWS_DensoPDF($orderno,$corno,$approvetype,$cusno,$path);
		// $file = "";
		if(empty($file) || $file===""){
			$subject = "**DENSO Order Mail Sent Error ".$corno;
			$content ="Dear Administrator<br>";
			...
			$content .="DENSO Sales() Co., Ltd.<br>";
			$content .="Customer Service & Operation";
			awssendEmaildenso($to,$subject,$content,$from,null);
			echo '{"result":"failPDF"}';
			return false;
		}
		foreach ($to as $email) {
	...
	if (!$mail->send()) {
		if(isset($bcc)){
			awssendmaildensoAttachFile($bcc ,$orderno , $corno , $cusno , $cusnm , null, $supno, $supnm, null);
		}
		
    	echo '{"result":"Failed = '.$mail->ErrorInfo.'"}';
	} else {
		if(isset($bcc)){
			$path_Backup = get_aws_config("path_PDF_AWS_Backup");
...
function awssendEmaildenso($to , $subject , $content , $from, $ccsup){
	$mail = new PHPMailer();
	$mail->IsHTML(true);
	$mail->IsSMTP(true);
	$mail->CharSet = "utf-8";
...
if(isset($_POST['axEmail']) && isset($_POST['orderNo']) && isset($_POST['corno']) 
	...
	awssendmaildensoAttachFile($to ,$orderno , $corno , $cusno , $cusnm ,$bcc, $supno, $supnm, $approvetype );
