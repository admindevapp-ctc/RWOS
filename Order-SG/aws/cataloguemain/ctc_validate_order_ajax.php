...
//check in orderhdr
$sql2 = "select * from orderhdr where CUST3 = '" . $dealer . "' and Corno='" . $vcorno . "' and Owner_Comp='$comp' Limit 1";  // edit by CTC
$sth2 = $ctcdb->db->prepare($sql2);
$sth2->execute();
$result2 = $sth2->fetchAll(PDO::FETCH_ASSOC);
$hasil2 = $result2[0];

//check in awsorderhdr
$sql4 = "select * from awsorderhdr where Cusno = '" . $vcusno . "' and Corno='" . $vcorno . "' and Owner_Comp='$comp' Limit 1";  // edit by CTC
...
if (!$hasil2 && !$hasil4) {
    ...
    if (!$hasil3) {
        ...
        foreach ($result as $r) {
            ctc_add_order_partno_aws($r['PartNo'], $vordno, $vcorno, $action, $vshpno, $oecus, $vshipment, $r['QTY'], $vordtype, $passDueDate);
        }
