...
//" where orderdtl.cust3='$cusno' and orderdtl.orderdate>='$datefrom' and orderdtl.orderdate<='$dateto' GROUP BY Product, subprod, orderdate, corno ORDER BY Product, subprod, orderdate, corno";
	...
	$sql = " SELECT  Product, Subprod, orderdate , Corno , sum( qty ) as ttlqty , curcd, sum( bprice * qty ) as ttlamount , sum( bprice * qty * SGPrice ) as ttlamountsg, orderno FROM orderdtl INNER JOIN bm008pr ON orderdtl.partno = bm008pr.itnbr AND orderdtl.Owner_Comp = bm008pr.Owner_Comp ".
...
$sQuery=  " SELECT  Product, Subprod, orderdate , Corno , sum( qty ) as ttlqty , curcd, sum( bprice * qty ) as ttlamount , sum( bprice * qty * SGPrice ) as ttlamountsg, orderno FROM orderdtl INNER JOIN bm008pr ON orderdtl.partno = bm008pr.itnbr AND orderdtl.Owner_Comp = bm008pr.Owner_Comp ".
" where orderdtl.cusno in $comp and orderdtl.orderdate>='$datefrom' and orderdtl.orderdate<='$dateto' and orderdtl.Owner_Comp='$owner_comp'  GROUP BY Product, subprod, orderdate, corno ORDER BY Product, subprod, orderdate, corno"	;
