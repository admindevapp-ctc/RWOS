<?php 
	...
	//total Row Count
	$sql = " SELECT  orderdtl.Owner_Comp,cusno, orderno, orderdate , Corno , sum( qty ) as ttlqty , curcd, sum( bprice * qty ) as ttlamount , sum( bprice * qty * SGPrice ) as ttlamountsg, orderno 
...
$sQuery=  " SELECT  orderdtl.Owner_Comp, cusno, orderno, orderdate , Corno , sum( qty ) as ttlqty , curcd, sum( bprice * qty ) as ttlamount , sum( bprice * qty * SGPrice ) as ttlamountsg, orderno 
FROM orderdtl  INNER JOIN bm008pr ON orderdtl.partno = bm008pr.itnbr AND orderdtl.Owner_Comp = bm008pr.Owner_Comp ".
" where  orderdtl.Owner_Comp='$comp' and orderdtl.orderdate>='$datefrom' and orderdtl.orderdate<='$dateto'";
if($vcussel!=''){
