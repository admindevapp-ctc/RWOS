...
/*Add new menu for AWS Sep 2022*/
function autofilldealer() {
    var type = $("input[name=vtype]").val();
	if(type == 'D'){
	   $("input[name=vdealer]").val($('[name=vcusno]').val());
	   $("input[name=vcust2]").val($('[name=vcusno]').val());
	   $("input[name=vcust3]").val($('[name=vcusno]').val());
	}else{
		$("input[name=vdealer]").val("");
		$("input[name=vcust2]").val($('[name=vcusno]').val());
		$("input[name=vcust3]").val($('[name=vcusno]').val());
	}
}
...
/*end comment AWS Sep 2022*/
	$(document).ready(function() {
	   $("input[name=vgroup]").val('DN');
...
/*end comment AWS Sep 2022*/
    ...
    <?php ctc_get_logo(); ?> <!-- add by CTC -->
		...
		if(trim($_GET['action'])=='add'){
			$inpcusno="<input type=\"text\" name=\"vcusno\" class=\"arial11blackbold\" maxlength=\"8\" size=\"9\" onkeyup=\"autofilldealer()\" required></input>";
			
			$inpname="<input type=\"text\" name=\"vcusnm\" class=\"arial11black\" maxlength=\"30\" size=\"30\" required></input>";
			...
			$inptype="<input type=\"text\"  name=\"vtype\" class=\"arial11black\" maxlength=\"10\" size=\"10\" onchange=\"resetdealer()\" required></input>";
			$inpalias="<input type=\"text\" name=\"valias\"  class=\"arial11black\" maxlength=\"2\" size=\"3\" required></input>";
			$inpgroup="<input type=\"text\"  name=\"vgroup\" class=\"arial11black\" maxlength=\"2\" size=\"3\" required></input>";
			$inpdealer="<input type=\"text\"  name=\"vdealer\" class=\"arial11black\" maxlength=\"8\" size=\"9\" required></input>";
      $inpcust2="<input type=\"text\"  name=\"vcust2\" class=\"arial11black\" maxlength=\"45\" size=\"45\" required> </input>";
			$inpcust3="<input type=\"text\"  name=\"vcust3\" class=\"arial11black\" maxlength=\"45\" size=\"45\" required></input>";
			...
			$inpoecus="<input type=\"text\" name=\"voecus\" maxlength=\"1\" size=\"2\" required value=".$oecus."></input>";
			$inproute="<input type=\"text\"  name=\"vroute\" class=\"arial11black\" maxlength=\"1\" size=\"2\" required></input>";
		}else{
		$vcusno=trim($_GET['cusno']);
		$query="select * from cusmas where Cusno='". $vcusno."' and Owner_Comp='".$comp."'";
		//echo $query;
      ...
      $inpaction="<input type=\"hidden\" name=\"vaction\" value=".$action."></input>";			
      $inpoecus="<input type=\"text\"  maxlength=\"1\" size=\"2\" name=\"voecus\" value=".$oecus."></input>";		
  ...
  <tr class="arial11blackbold">
    ...
    <td class="arial11redbold">(A=2<sup>nd</sup> customer, D=1<sup>st</sup> customer CD)</td>
    <!-- end comment -->
  ...
  <tr class="arial11blackbold">
    ...
    <td>1<sup>st</sup> customer CD</td>
    <!-- end comment -->
    <td>:</td>
    <td><? echo $inpdealer ?></td>
    <td>OE Customer</td>
    <td>&nbsp;</td>
    <td><? echo $inpoecus ?> Y or N (Y=OE)</td>
  </tr>
  <tr class="arial11blackbold">
    <td>&nbsp;</td>
    <td>&nbsp;</td>
